<template>
    <div class="information-publish-wrapper">
        ...
        <div class="form-content">
            <Form ref="informationForm" :model="informationDataInfo" :rules="imformationValidateRules" :label-width="80" label-position="left">
                <Row>
                    <Col span="24">
                        <FormItem label="资料类型">
                            <div class="form-item-sm-wrap">
                                <RadioGroup v-model="informationDataInfo.upperChannelType" @on-change="changeModuleType">
                                    <Radio :label="item.channelCode" v-for="(item, i) of moduleList" :key="i">{{ item.channelName }}</Radio>
                                </RadioGroup>
                            </div>
                        </FormItem>
                    </Col>
                ...
                <Row>
                    <Col span="24">
                        <FormItem label="标题" class="flex-block" prop="title">
                            ...
                            <div class="ext">
                                <Checkbox v-model="informationDataInfo.isBold" :true-value="1" :false-value="0" class="is-blod">加粗标红</Checkbox>
                            </div>
                        </FormItem>
                    </Col>
                </Row>
                    ...
                    <Col span="24">
                        <FormItem label="计划类目">
                            <RadioGroup v-model="informationDataInfo.channeltype">
                                <Radio :label="item.channelCode" v-for="(item, i) of childPlanType" :key="i">{{ item.channelName }}</Radio>
                            </RadioGroup>
                        </FormItem>
                    </Col>
                </Row>
        ...
        methods: {
            changeModuleType(evt, flag) {
                ...
                if(moduleItem.length){
                    ...
                    this.getThirdChannelData({upperCode: module.channelCode}).then(res => {
                        if(res.success){
            ...
            reloadData(id){
                ...
                this.getDataInfoDetail({id}).then(res => {
                    if(res.success){
                        this.informationDataInfo = Object.assign(this.informationDataInfo, res.data);
                        ...
                        this.changeModuleType(this.informationDataInfo.upperChannelType, true);
                    }
                })
            },
            ...mapActions([

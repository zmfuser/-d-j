    ...
    <div class="worl-plan-list-warp">
        <listHeader :listTitle="listTitle" :subTitle="subTitle" openMarginBotton>
            <div class="slot-opts-wrap workplan-opts" slot="headOpts" >
                <div class="opt-item" @click="lookDept" v-show="!isViewDeptData && !hidden" v-if="!isZBManager">
                    <div class="icon workplan-icon"></div>
                    <div class="title">部门工作计划</div>
                </div>
                <div class="opt-item" @click="createWorkPlan" v-show="!isNormalEmp && (isViewDeptData || isZBManager)">
                    <div class="icon workplan-icon"></div>
                    <div class="title">新增工作计划</div>
                </div>
                <div class="opt-item no-border" v-if="isViewDeptData">
                    <filterControler channelAction="getWorkPlanChannel" :channelParams="actionParams" isMyDeptView></filterControler>
                </div>
            </div>
        </listHeader>
        <tabsBar :tabsData="tabsData" @tabChange="tabChange" noMarginBottom defaultBgColor="#ebebeb" tabsType="workplan" v-if="showTabsBar"></tabsBar>

        <div class="status-tab-bar" v-if="!showTabsBar">
            <div class="tab-bar" v-for="(item, index) in allTabbars" :key="index" @click="handleCurTabbar(item, index)" :class="{'cur-tab-bar': index === curTabbar}">
        ...
        <dataList :itemDelSwitch="showCheckBox" :actionUrl="actionUrl" :actionParams="actionParams" :countMapper="currentTab" openFilter>
            <template v-slot:itemOpts="{dataItem}" v-if="isViewDeptData || isZBManager">
                <div class="slot-opts-wrap edit-item-opt">
                    <div class="opt-item change" @click="gotoEditForChange(dataItem)" v-show="!isNormalEmp && dataItem.showEditButton === 1 && curTabbar === 0">
                        <div class="title">
                            <img src="./../../images/gl_icon_bj.png" alt="">
                            变更
                        </div>
                    </div>
                    <div class="opt-item change" @click="gotoReport(dataItem)" v-show="!isNormalEmp && dataItem.workplanstatus == '2'">
                        <div class="title">
                            <img src="./../../images/gl_icon_bj.png" alt="">
                            汇报
                        </div>
                    </div>
                    <div class="opt-item change" @click="handleRepublish(dataItem)" v-show="!isNormalEmp && dataItem.workplanstatus == '4'">
                        <div class="title">
                            <!-- <img src="./../../images/gl_icon_bj.png" alt=""> -->
                            重新发布
                        </div>
        ...
        <Modal v-model="reportShow" title="汇报" @on-cancel="handleCancelReport" :mask-closable="false">
            <Form :model="reportForm" :label-width="130" label-position="left">
                <!-- <FormItem label="状态变更">
                    <Radio v-model="reportForm.workPlanStatus">未执行</Radio>
                </FormItem> -->
                <FormItem label="计划执行情况">
                    <el-input type="textarea" :rows="4" maxlength="800" v-model="reportForm.planDetail" show-word-limit></el-input>
                </FormItem>
                <FormItem label="上传图片">
                    <el-upload
    ...
    export default {
        ...
        methods: {
            ...
            gotoEditForChange(data) {
                ...
                this.changeSubNaviPath('workPlanUpload');
                this.$router.push({name: 'workPlanUpload', params: {id: data.id, action: 'change'}})
            },
            // 汇报工作
            gotoReport(data) {
            ...
            handleImgDelete(file) {
                console.log(file);
                this.reportForm.imgList.splice(this.reportForm.imgList.indexOf(file.response.data.ids[0]), 1);
            ...
            handleFileDelete(file) {
                console.log(file);
                this.reportForm.fileList.splice(this.reportForm.fileList.indexOf(file.response.data.ids[0]), 1);

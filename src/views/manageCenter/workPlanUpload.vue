        ...
        <Row>
          <Col span="24">
            <FormItem label="选择所属组织" prop="ownedOrgId"    >
              <i-select v-model="workPlanInfo.ownedOrgId" style="width:200px">
                <i-option
          ...
          <Col span="24">
            <FormItem label="开始执行时间" prop="execTime">
              <Date-picker
                type="date"
                format="yyyy-MM-dd"
                :value="workPlanInfo.execTime"
                :options="execTimeOptions"
        ...
        <Row v-if="isChange" style="margin-top: 20px">
          <Col span="24">
            <FormItem label="变更理由" prop="changeReason">
              <Input
                v-model="workPlanInfo.changeReason"
                type="textarea"
                :rows="3"
                placeholder="请输入变更理由"
              />
    ...
    reloadData(id) {
      ...
      this.getDataInfoDetail({ id }).then(res => {
        if (res.success) {
          ...
          this.workPlanInfo.ownedOrgId=res.data.essayOrgId;

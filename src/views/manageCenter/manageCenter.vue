<template>
  <div class="manage-center-warp">
    ...
    <div class="item-wrapper item-wrapper1" v-if="itemList.length >3">
      <div
        class="manage-center-item"
        v-for="(item,index) in itemList.slice(3, 6)"
        :key="index"
        @click="cliclRow(item)"
      >
        <div :class="`item-icon ${item.icon}`"></div>
    ...
    <div class="item-wrapper item-wrapper2" v-if="itemList.length > 6">
      <div
        class="manage-center-item"
        v-for="(item,index) in itemList.slice(6)"
        :key="index"
        @click="cliclRow(item)"
      >
        <div :class="`item-icon ${item.icon}`"></div>
...
export default {
  ...
  computed: {
    ...
    ...mapGetters(["getCurrentNavi", "getLoginUserInfo"])
    // getLoginUserInfo(){
    //     this.splitGetuserinfo();
    // }
  },
  ...
  methods: {
    splitGetuserinfo() {
      ...
      if (powerCodes) {
        ...
        if (!result) {
          this.itemList = this.itemList.splice(2);
        }
      }
    },
    cliclRow(item) {
    ...
    ...mapActions(["changeNaviPath", "changeSubNaviPath"])
  }
  // watch: {
  //     getLoginUserInfo(){

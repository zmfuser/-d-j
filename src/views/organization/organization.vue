    ...
    <div class="organization-view-wrapper">
        <div class="header-wrap">
            ...
            <div class="org-opts-wrap" v-show="!isNormalEmp">
                <Button class="btn-item" :disabled="globalEditMode" @click="openCreateOrgMode" v-show="!(isNormalEmp || isZBManager)">
                ...
                <Button class="btn-item" :disabled="globalEditMode" @click="openPersonMove" v-if="isJTManager || isSysManager">
                    <div class="move-icon"></div>
                    人员移动
                </Button>
                <Button class="btn-item" :disabled="globalEditMode" @click="openOrgDrag" v-if="isJTManager || isSysManager">
        ...
        <div class="org-view-container">
            <div class="root-node-wrap">
                <div class="node-info">
                    <div class="title jt-title" :title="orgDataCpd.orgname" :class="orgDataCpd.editMode?'whiteBg':''" @click="orgRename(orgDataCpd)">{{ orgDataCpd.orgname }}</div>
            ...
            <div class="children-node-wrap" v-show="orgDataCpd.isShow">
                <div class="border-container">
                    <div class="node-item-wrap margin-b-50" v-for="(secNode, sec) of orgDataCpd.orgList" :key="sec">
                        <div class="node-info margin-r-26" :class="sec == 0?'level-first-node':'level-middle-node'">
                            <div class="title" :title="secNode.orgname" :class="secNode.editMode?'whiteBg':''" @click="orgRename(secNode)">
                        ...
                        <div class="border-container" v-show="secNode.isShow">
                            <div class="node-item-wrap width-686" v-for="(thiNode, thi) of secNode.orgList" :key="thi">
                                <div class="node-info" :class="thi == 0?'level-first-node':'level-middle-node'">
                                    <div class="title" :title="thiNode.orgname" :class="(thiNode.editMode) ?'whiteBg':''" @click="orgRename(thiNode)">
                                        <span>{{ thiNode.orgname }}</span>
                                    </div>
                                    <div class="delete-node" v-if="thiNode.editMode && !isZBManager && thiNode.orgname != secNode.orgList[0].orgname" @click="deleteNode(thiNode)"></div>
                                    ...
                                    <div class="node-members">
                                        <template v-if="!isNormalEmp && (getLoginUserInfo.oid == (thiNode.orgname == secNode.orgList[0].orgname ? thiNode.upperid : thiNode.id))">
                                        ...
                                        <template v-else-if="(!isNormalEmp && (getLoginUserInfo.oid == thiNode.upperid || getLoginUserInfo.orgDept.orgcode+2 == thiNode.orgcode)&& !isZBManager) || isSysManager || isJTManager">
                                ...
                                <template v-if="thiNode.empList.length && thiNode.isShow">
                                    <div class="all-member-content">
                                        <div class="member-inner" >
                                            <CheckboxGroup v-model="thiNode.deleteMemberList">
                                                <div class="member-wrap" :class="thiNode.editMemberMode?'edit-mode':''" v-for="(m, i) of thiNode.empList" :key="i">
                                    ...
                                    <div class="member-edit-wrap" v-if="(!isNormalEmp && (getLoginUserInfo.oid == (thiNode.orgname == secNode.orgList[0].orgname ? thiNode.upperid : thiNode.id)) || isSysManager)">
                                        <template v-if="!thiNode.editMemberMode">
                                            <div class="member-edit-btn" :class="globalEditMode?'disabled':''" @click="openMemberEditMode(thiNode)">
                                                <div class="icon edit-icon"></div>
                                                <div class="label">编辑</div>
                                            </div>
                                            <div class="member-edit-btn" :class="globalEditMode?'disabled':''" @click="openMemberDelMode(thiNode)">
        ...
        <Modal v-model="editMemberFlag" title="编辑人员" :width="720" class="member-edit-modal" :mask-closable="false">
            <div class="modal-content-back">
                <Form ref="editMemberForm" :model="editMemberForm" :rules="memberValidateRules" label-position="left" :label-width="160">
                    <Row>
                        <Col span="24">
                            <FormItem label="选择组织人名">
                                <div class="member-list-wrap">
                                    <div class="member-item" :class="(item.isChecked || item.isEdited)?'active':''" v-for="(item, i) of editMemberForm.empList" :key="i" @click="memberEditCheck(item, i)">
                                        <span>{{ item.name }}</span>
                                    </div>
                                </div>
                            </FormItem>
                        ...
                        <Col span="24">
                            <FormItem label="输入修改后名字" prop="newName">
                                <div class="edit-input-wrap">
                                    <Input v-model="editMemberForm.newName" :maxlength="5" @on-blur="saveMemberEditor('name')"></Input>
                                </div>
                            </FormItem>
                        </Col>
                    </Row>
        ...
        <Modal v-model="createOrgModalFlag" title="添加部门" :width="720" class="create-org-modal" :mask-closable="false">
            <div class="modal-content-back">
                <Form ref="createOrgForm" :model="createOrgForm" :rules="orgValidateRules" label-position="left" :label-width="140">
                    <div class="top">
                        <Row>
                            <Col span="24">
                                <FormItem label="选择上级组织" prop="parentOrg">
                                    <div class="input-wrap">
                                        <Select v-model="createOrgForm.parentOrg">
                                            <Option v-for="(item, i) of parentOrgList" :value="item.id" :key="i">{{ item.orgname }}</Option>
                                        </Select>
                                    </div>
                                </FormItem>
                            </Col>
        ...
        <Modal v-model="createMemberModalFlag" title="添加人员" :width="720" class="create-member-modal" :mask-closable="false" :transfer="true">
            <div class="modal-content-back">
                <Form ref="createMemberForm" :model="createMemberForm" :rules="orgValidateRules" label-position="left" :label-width="140">
                    <div class="top">
                        <Row>
                            <Col span="24">
                                <FormItem label="选择组织" prop="parentOrg">
                                    <div class="input-wrap">
                                        <Select v-model="createMemberForm.parentOrg" label-in-value @on-change="parentOrgChange">
                                            <Option v-for="(item, i) of parentOrgList2" :value="item.id" :key="i">{{ item.orgname }}</Option>
                                        </Select>
                                    </div>
                                </FormItem>
                            </Col>
                        ...
                        <Row>
                            <Col span="24">
                                <FormItem label="输入组织人名" prop="empname">
                                    <div class="input-wrap">
                                        <Input v-model="createMemberForm.empname" :maxlength="4" @on-blur="addMember2('name')"></Input>
                                    </div>
                                </FormItem>
                            </Col>
                        </Row>
            ...
            <div class="modal-content-back">
                <div class="jt-member-list-wrap">
                    <CheckboxGroup v-model="orgData.deleteMemberList">
                        <div class="member-wrap" :class="jtMemberEditFlag?'edit-mode':''" v-for="(m, i) of orgData.empList" :key="i">
        ...
        <Modal v-model="editJTMemberFlag" title="编辑机关人员" :width="720" class="member-edit-modal" :mask-closable="false">
            <div class="modal-content-back">
                <Form ref="editJTMemberForm" :model="editJTMemberForm" :rules="memberValidateRules" label-position="left" :label-width="160">
                    <Row>
                        <Col span="24">
                            <FormItem label="选择组织人名">
                                <div class="member-list-wrap">
                                    <div class="member-item" :class="(item.isChecked || item.isEdited)?'active':''" v-for="(item, i) of orgData.empList" :key="i" @click="jtMemberEditCheck(item, i)">
                                        <span>{{ item.name }}</span>
                                    </div>
                                </div>
                            </FormItem>
                    ...
                    <Row>
                        <Col span="24">
                            <FormItem label="输入修改后名字" prop="newName">
                                <div class="edit-input-wrap">
                                    <Input v-model="editJTMemberForm.newName" :maxlength="5" @on-blur="saveMemberEditor2('name')"></Input>
                                </div>
                            </FormItem>
                        </Col>
                    </Row>
                    <Row>
                        <Col span="24">
                            <FormItem label="输入修改后电话号码" prop="newPhone">
                                <div class="edit-input-wrap">
                                    <Input v-model="editJTMemberForm.newAccount" @on-blur="saveMemberEditor2('account')"></Input>
                                </div>
                            </FormItem>
                        </Col>
                    </Row>
        ...
        <Modal v-model="orgRenameModal" title="修改部门名称" :width="720" class="org-rename-modal" @on-ok="orgRenameSubmit">
            <div class="modal-content-back">
                <Form ref="orgRenameForm" :model="orgRenameForm" :rules="orgValidateRules" label-position="left" :label-width="160">
                    <Row>
                        <Col span="24">
		...
		<person-move :person-move-show="personMoveShow" @closePersonMove="closePersonMove" @refresh="refreshOrgData"></person-move>
    </div>
</template>
    ...
    export default {
        ...
        data() {
            ...
            return {
                ...
                orgValidateRules: {
                    parentOrg: [
                        { required: true, type: "number", message: '请选择上级组织', trigger: 'blur' }
                    ],
                    orgname: [
        ...
        methods: {
            ...
            confirmEditInfo(){
                if(this.editMemberForm.newAccount !="" || this.editMemberForm.newName != ''){
                    return;
                }
            ...
            confirmEditInfo2(){
                if(this.editJTMemberForm.newAccount !="" || this.editJTMemberForm.newName != ''){
                    return;
                }
            ...
            validateForm(){
                if(this.createOrgForm.empAccount !="" || this.createOrgForm.empname != ''){
                    return;
                }
                ...
                this.$nextTick(() => {
                    this.$refs.createOrgForm.validate((valid) => {
                        if(valid){
                            ...
                            let orgcode = '';
                            let employees = [];
                            orgcode = this.parentOrgList.filter(item => item.id == parentOrg)[0].orgcode;
            ...
            validateForm2(){
                if(this.createMemberForm.empAccount !="" || this.createMemberForm.empname != ''){
                    return;
                }
                if(!this.createMemberForm.empList.length){
                    this.$Modal.error({
            ...
            closeModal(ref){
                if(ref == 'editMemberForm'){
                    ...
                    this.createMemberForm = util.initializationObject(this.createMemberForm);
            ...
            saveMemberEditor(triggerTarget){
                let {memberId, memberOrgid, memberOrgname, newName, newAccount, mapIndex} = this.editMemberForm;
                ...
                this.$refs.editMemberForm.validateField('newPhone', (valid) => {
                    if(valid == ''){
                        if(newName && newAccount){
                            let filterList = this.editMemberForm.addMemberList.filter(item => item.id == memberId);
            ...
            saveMemberEditor2(triggerTarget){
                this.$refs.editJTMemberForm.validateField('newPhone', (valid) => {
                    if(valid == ''){
                        let {memberId, memberOrgid, memberOrgname, newName, newAccount, mapIndex} = this.editJTMemberForm;
                        if(newName && newAccount){
                            let filterList = this.editJTMemberForm.addMemberList.filter(item => item.id == memberId);
                            if(filterList.length == 0){
            ...
            deleteJTMember(){
                ...
                this.$Modal.confirm({
                    ...
                    loading: true,
                    onOk(){
                        let employees = util.deepClone(_this.orgData.deleteMemberList).map(item => {
                            return {id: item};
                        });
                        let params = {
                            orgStatus: 0,
            ...
            watchWorkPlan(node){
                ...
                this.$router.push({ name: 'workPlanList', params: {type: 'deptWorkPlan', orgId: node.id}});
            },
            myWorkPlan(){
                this.changeNaviPath('manageCenter');
                this.changeSubNaviPath('workPlanList');
                this.$router.push({name: 'workPlanList', params: {type: 'myworkplan'}});
            },
            orgRename(node){
                // if(node.orgname == '党委'){
                //     return;
...
<style lang="less" scoped>
    ...
    .create-org-modal, .create-member-modal, .member-edit-modal {
        .modal-content-back {
            padding: 0 14px;
            .input-wrap {
                width: 256px;
            }
            .add-member-wrap, .edited-member-list-wrap {
                display: flex;
                justify-content: flex-start;
                flex-flow: wrap;
                .add-member-item, .edited-member {
                    position: relative;
                    min-width: 116px;
                    padding: 4px 0;
                    margin-right: 18px;
...
.ivu-select-dropdown-list {
    ...
    .create-org-modal, .member-edit-modal, .create-member-modal {
